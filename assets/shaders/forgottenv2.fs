...
extern PRECISION vec2 forgottenv2;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float weight = 0.07 + sin(forgottenv2.x) * 0.08;
	//float wacky = 
	for (int i = -2; i < 2; i++) {
		for (int j = -2; j < 2; j++) {
			pixel -= weight * (vec4(0.999) - Texel(texture, texture_coords + vec2(i / image_details.x, j / image_details.y)));
			float bwavg = (pixel.r + pixel.g + pixel.b)/3;
			//pixel = vec4(((0.5*bwavg)+ (0.5*(190./255.))), ((0.5*bwavg)+ (0.5*(90./255.))), ((0.5*bwavg)+ (0.5*(56./255.))), 0.);
			
		}
	}
	pixel = 0.2*pixel + 0.8*(vec4(190./255., 90./255.,56./255.,0.));

	return dissolve_mask(vec4(pixel.rgb, a), texture_coords, uv);
}
