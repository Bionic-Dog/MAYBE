...
// Card rotation
extern PRECISION vec2 investment;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float pixel_size = 1 / max(texture_details.z, texture_details.a);

    bool dithering = getValue((hsl.z) *.5+.5, uv, pixel_size);
   	vec4 final_pixel = vec4(vec3(!dithering), pixel.a);

//	hsl.z = hsl.z > 0.5 + sin(time) * 0.1? 1 : 0.2;
//	vec4 final_pixel = RGB(hsl);

	if (final_pixel.a > 9999) final_pixel.a = investment.x;
    
    vec4 ret = Texel(texture, texture_coords);

    vec4 colour_1 = vec4(191./255., 204./255., 152./255., final_pixel.a);  // dollar
    vec4 colour_2 = vec4(99./255., 115./255., 74./255., final_pixel.a);  // d2llar

    float bwavg = ((final_pixel * colour).r + (final_pixel * colour).g + (final_pixel * colour).b)/3;
    if(bwavg >= 0.5) ret = colour_1;
    if(bwavg < 0.5) ret = colour_2;
    
    
	return dissolve_mask((0.001*(final_pixel * colour)+(0.999*(ret))), texture_coords, uv);
}
